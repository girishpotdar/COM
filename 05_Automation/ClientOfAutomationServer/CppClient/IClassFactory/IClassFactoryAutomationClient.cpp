...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg) {
	case WM_CREATE:
		hr = CoCreateInstance(CLSID_MyMath, NULL, CLSCTX_INPROC_SERVER, IID_IMyMath, (void**)&gpIMyMath);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("IMyMath interface cannot be obtained."), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
		}
		...
		gpIMyMath->SumOfTwoIntegers(iNum1, iNum2, &iSum);
		...
		gpIMyMath->SubtractionOfTwoIntegers(iNum1, iNum2, &iSub);
		...
		// as IMyMath is now not needed onwards, release it
		gpIMyMath->Release();
		gpIMyMath = NULL;	//make released interface NULL
