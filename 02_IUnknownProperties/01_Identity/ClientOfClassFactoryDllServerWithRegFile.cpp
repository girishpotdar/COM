...
IUnknown *gpIUnknownFromISum;
IUnknown *gpIUnknownFromISubtract;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		gpISum->QueryInterface(IID_IUnknown, (void**)&gpIUnknownFromISum);
		gpISubtract->QueryInterface(IID_IUnknown, (void**)&gpIUnknownFromISubtract);

		if (gpIUnknownFromISum == gpIUnknownFromISubtract)
		{
			MessageBox(hwnd, TEXT("Both IUnknown are from same component"), TEXT("OK"), MB_OK);
		}
		else
		{
			MessageBox(hwnd, TEXT("Both IUnknown are from different component"), TEXT("Error"), MB_OK);
		}
		gpISum->Release();
		gpISum = NULL;
